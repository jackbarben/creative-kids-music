...
export default function WorkshopRegistrationForm({ workshops }: WorkshopRegistrationFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-8">
        ...
        {selectedWorkshops.length > 0 && (
          <div className="mb-6 p-4 bg-forest-50 rounded-lg">
            <p className="text-stone-700">
              {childCount} {childCount === 1 ? t('shared.child') : t('shared.children')} × {selectedWorkshops.length} {selectedWorkshops.length === 1 ? t('shared.workshop') : t('shared.workshopPlural')} = <span className="font-bold text-forest-700">${calculateTotal()}</span>
              {childCount > 1 && (
                <span className="text-sm text-forest-600 ml-2">{t('shared.includesSiblingDiscount')}</span>
              )}
            </p>
          </div>
        )}
      ...
      <div className="pt-4">
        <SubmitButton>{t('workshop.register')}</SubmitButton>
      </div>
    </form>
  )
}
