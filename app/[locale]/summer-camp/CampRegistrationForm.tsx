    ...
    <form action={formAction} className="space-y-8">
      ...
      <section>
        ...
        <div className="mb-6 p-4 bg-terracotta-50 rounded-lg">
          <p className="text-stone-700">
            {childCount} {childCount === 1 ? t('shared.child') : t('shared.children')} = <span className="font-bold text-terracotta-700">${calculateTotal()}</span>
            {childCount > 1 && (
              <span className="text-sm text-terracotta-600 ml-2">{t('shared.includesSiblingDiscount')}</span>
            )}
          </p>
        </div>

        <FormRadioGroup
          label={t('shared.payment')}
          name="payment_preference"
          required
          options={[
            {
              value: 'later',
              label: t('shared.standardPayment'),
              description: t('shared.standardPaymentDesc'),
            },
            {
              value: 'assistance',
              label: t('shared.requestAssistance'),
              description: t('shared.requestAssistanceDesc'),
            },
          ]}
        ...
        <FormTextarea
          label={t('shared.assistanceNotes')}
          name="assistance_notes"
          className="mt-4"
          placeholder={t('shared.assistanceNotesPlaceholder')}
        />
      </section>
      ...
      <section className="border-t border-stone-200 pt-8">
        ...
        <div className="space-y-6">
          <div>
            <FormSelect
              label={t('shared.howHeard')}
              name="how_heard"
              options={[
                { value: '', label: t('shared.howHeardSelect') },
                { value: 'friend', label: t('shared.howHeardOptions.friend') },
                { value: 'church', label: t('shared.howHeardOptions.church') },
                { value: 'school', label: t('shared.howHeardOptions.school') },
                { value: 'social', label: t('shared.howHeardOptions.social') },
                { value: 'search', label: t('shared.howHeardOptions.search') },
                { value: 'workshop', label: t('shared.howHeardOptions.workshop') },
                { value: 'other', label: t('shared.howHeardOptions.other') },
              ]}
            ...
            {howHeard === 'other' && (
              <div className="mt-3">
                <label className="block text-sm font-medium text-stone-700 mb-1">
                  {t('shared.pleaseSpecify')}
                </label>
                <input
                  type="text"
                  name="how_heard_other"
                  placeholder={t('shared.howHeardPlaceholder')}
          ...
          <FormTextarea
            label={t('shared.excitedAbout')}
            name="excited_about"
            rows={2}
          />
          <FormTextarea
            label={t('shared.anythingElse')}
            name="message"
            rows={2}
          />
        </div>
      ...
      <div className="pt-4">
        <SubmitButton>{t('camp.register')}</SubmitButton>
      </div>
    </form>
  )
}
