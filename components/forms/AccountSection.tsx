    ...
    <section>
        ...
        <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-800 font-medium">{t('signedInAs')} {user.email}</span>
            </div>
            <button
              type="button"
      ...
      {!isLoggedIn && !loading && (
        <p className="text-sm text-slate-500">
          {t.rich('signInFirst', {
            link: (chunks) => (
              <a href="/account" className="text-forest-600 hover:text-forest-700 underline">{chunks}</a>
            ),
          })}
        </p>
      )}
